...
class CRF(nn.Module):
    def __init__(self, tagset_size):  # average_batch=False,
        ...
        init_transitions = torch.rand(self.tagset_size + 2, self.tagset_size + 2)
        ...
        if torch.cuda.is_available():
            ...
            ratio = ratio.cuda()
        self.ratio = nn.Parameter(ratio, requires_grad=True)
    ...
    def _calculate_PZ(self, feats, mask):
        ...
        scores = feats + self.ratio * self.transitions.view(1, tag_size, tag_size).expand(ins_num, tag_size, tag_size)
